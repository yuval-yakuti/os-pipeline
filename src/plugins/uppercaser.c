...
typedef struct upper_state {
    string_bq *q;
    pthread_t thread;
    const char* (*next_place)(const char*);
} upper_state;

static upper_state G = {0};
...
const char* plugin_init(int queue_size) {
    ...
    if (!G.q) return "uppercaser: queue create failed";
    if (pthread_create(&G.thread, NULL, worker, NULL) != 0) return "uppercaser: thread create failed";
    return NULL;
}
...
const char* plugin_place_work(const char* str) {
    if (!G.q) return "uppercaser: not initialized";
    if (str == BQ_END_SENTINEL || (str && strcmp(str, "<END>") == 0)) {
        return bq_push(G.q, (char *)BQ_END_SENTINEL) == 0 ? NULL : "uppercaser: push failed";
    }
    char *dup = dup_cstr(str ? str : "");
    if (!dup) return "uppercaser: OOM";
    return bq_push(G.q, dup) == 0 ? NULL : "uppercaser: push failed";
}
